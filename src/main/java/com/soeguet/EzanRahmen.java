...
public class EzanRahmen {
    ...
    private final HashMap<String, NamazVaktiRecord> hashMap = new LinkedHashMap<>();
    private final Color markiertFarbe = new Color(0x930C191E, false);
    private NamazVaktiRecord[] namazVakti;
    ...
    private String heutigerEintrag() {
...
//        namazVakti = eintraege(httpResponse);
    ...
    private void initScreen() {
        ...
        String wert = heutigerEintrag();
        datumComboBox.setSelectedItem(wert);

        ogle = hashMap.get(wert).ogle();
        ikindi = hashMap.get(wert).ikindi();
        aksam = hashMap.get(wert).aksam();
        sabah = hashMap.get(wert).gunes();
        yatsi = hashMap.get(wert).yatsi();
    ...
    private void comboElemente() {
        ...
        for (NamazVaktiRecord gebetszeit : namazVakti) {

            hashMap.put(gebetszeit.datum(), gebetszeit);
            defaultComboBoxModel.addElement(gebetszeit.datum());
        }

        datumComboBox.setModel(defaultComboBoxModel);
    }

    private NamazVaktiRecord[] eintraege(HttpResponse<String> httpResponse) {
...
//        String content = httpResponse.body();
        ...
        try {
            return objectMapper.readValue(content, NamazVaktiRecord[].class);
        }
